...
\
      if (data.schedule) {\
        ...
        let newSlots: TimetableSlot[] = gen.timeSlots.map((ts: any, idx: number) => ({\
        ...
        if (gen.overallAdvice) setAiAdvice(gen.overallAdvice);\
        if (gen.revisionMilestones) setMilestones(gen.revisionMilestones);\
      }\
    } catch (err) {\
      console.error(err);\
